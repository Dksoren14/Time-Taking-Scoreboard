/* autogenerated by Processing revision 1296 on 2025-08-02 */
...
public class S_rensTidstager extends PApplet {
...
public void mouseReleased() {                      //Runs once when mouse is released.
...
public void mousePressed() { //mousePressed is a built-in function that is called once every time a mouse button is pressed.
...
public void take_time() {
  ...
  textSize(50);
  text(currentTime * 0.001f, 850, 375);
  popStyle();
}
public void display_S_Time() {
  if (stop_Time) {
    pushStyle();
    fill(255); // White background for the rect
    noStroke();
    ...
    textAlign(CENTER, CENTER);
    textSize(50);
    text(stopped_Time * 0.001f, 850, 375);
    popStyle();
  }
}
...
public void display_Times() {
  ...
  try {
    text((times.get(0))*0.001f, 1450, 150+(0*60));
    text((times.get(1))*0.001f, 1450, 150+(1*60));
    text((times.get(2))*0.001f, 1450, 150+(2*60));
    text((times.get(3))*0.001f, 1450, 150+(3*60));
    text((times.get(4))*0.001f, 1450, 150+(4*60));
    text((times.get(5))*0.001f, 1450, 150+(5*60));
    text((times.get(6))*0.001f, 1450, 150+(6*60));
    text((times.get(7))*0.001f, 1450, 150+(7*60));
    text((times.get(8))*0.001f, 1450, 150+(8*60));
    text((times.get(9))*0.001f, 1450, 150+(9*60));
  }
  catch(Exception e) {
    //  Block of code to handle errors
  }
...
public void insert_manueltB() {
  if (add_TimeM_Bool) {
    boolean insertedM = false;

    for (int i = 0; i < manuel_Times.size(); i++) {
      if (PApplet.parseFloat(manuel_tidsInput.getText()) < manuel_Times.get(i)) {
        manuel_Times.add(i, PApplet.parseFloat(manuel_tidsInput.getText()));
    ...
    if (!insertedM) {
      // If it's the largest time, add it to the end
      manuel_Times.add(PApplet.parseFloat(manuel_tidsInput.getText()));
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "S_rensTidstager" };
    if (passedArgs != null) {
